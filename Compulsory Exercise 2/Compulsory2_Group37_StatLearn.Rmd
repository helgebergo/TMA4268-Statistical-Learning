...
summary(d.train)
str(d.train)
plot(glu,diabetes,col="red")
points(bmi,diabetes,col="blue")
plot(npreg)
plot(log(npreg))
max(npreg)
...
```{r}
svmfit = svm(diabetes~., data=d.train, kernel="linear", cost = 0.1)

set.seed(1)
tune.out.linear = tune(svm,diabetes~.,data=d.train,kernel="linear", ranges = list(cost=c(0.001,0.01,0.1,1,5,10,100)))

best.mod = tune.out.linear$best.model
summary(best.mod)

ypred  = predict(best.mod,d.test)
table(predict=ypred, truth = d.test$diabetes)

```


```{r}
tune.out.radial = tune(svm,diabetes~.,data=d.train,kernel="radial", ranges = list(cost=c(0.001,0.01,0.1,1,5,10,100)))
