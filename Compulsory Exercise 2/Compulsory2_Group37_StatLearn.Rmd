...
points(max_point,reg.summary$adjr2[max_point], col="black",cex=2,pch=20)
```


The maximum `adjusted` $R^2$ is the one with `r max_point` variables, with a value of `r reg.summary$adjr2[max_point]`, shown as a filled dot in the lower right plot. However, all the plots are pretty flat after around 6 or 7 variables used, and it seems like using only 6 variables still gives a good `adjusted` $R^2$ value of `r reg.summary$adjr2[6]`, without the increased complexity of adding 7 more variables. The model is then:
...
The MSE of the model with 6 variables is `r val.errors[6]`, when using the model on the test set.  
