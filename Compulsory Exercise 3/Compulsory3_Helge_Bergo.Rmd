...
```{r, include=FALSE}
# train.ind = sample(1:nrow(x.train), 0.2 * nrow(x.train))
# x.val = x.train[train.ind, ]
# x.train = x.train[-train.ind, ]
# y.val = y.train[train.ind]
# y.train = y.train[-train.ind]
```
...
```{r 1cplotting, fig.width=6, fig.height=4}
plot(history)
```
...
The final MSE after training the model for 300 epochs was `r round(score,2)`. Compared to the MSE of the methods from Compulsory 2, this is a relatively good MSE score, and it is lower than several of the methods, like forward selection, lasso, and splines, but not as low as bagging and the random forest. 
...
```{r 1d, cache = TRUE, fig.width=6, fig.height=4}
...
score_drop_03_reg001 <- score_reg
```

After implementing both dropout and regularization
The final MSE after 300 epochs was `r round(score_reg,digits = 2)`.
